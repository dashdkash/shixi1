"""
LangGraph 状态图 — 并行多 Agent 工作流编排

架构：
  用户输入 → Supervisor（路由）→ [Detection, Analysis, QA]（并行 fan-out）→ Summarize（fan-in）→ END

并行机制：
  - Supervisor.decide_next() 返回列表，如 ["detection", "analysis"]
  - LangGraph 自动对列表中的节点执行 fan-out（并行）
  - 所有并行节点完成后触发 summarize（fan-in）
...
def build_multi_agent_graph():
    ...
    # 条件路由：Supervisor.decide_next() 返回列表 → LangGraph 自动 fan-out 并行
    ...
    # 各 Agent 执行完毕后进入汇总（fan-in：所有并行节点完成后才触发 summarize）
    ...
    logger.info("LangGraph 并行多 Agent 状态图构建完成（支持 fan-out/fan-in）")
