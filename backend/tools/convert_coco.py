#!/usr/bin/env python3
# backend/tools/convert_coco.py
"""
COCO JSON -> YOLO TXT 数据集格式转换脚本

功能：
1. 将 COCO 格式的 JSON 标注文件转换为 YOLO 格式的 TXT 标注文件
...
# 类别映射（格式：{类别名称: 类别ID}）
# 如果设为 None，则按 COCO categories 中的顺序自动编号
CLASS_MAPPING = {
    "aircraft": 0,
    "oiltank": 1,
    "overpass": 2,
...
def coco_to_yolo(json_file: str, output_dir: str, class_mapping: dict | None = None) -> dict:
...
def main():
    """主函数"""
    print("=" * 70)
    print(" COCO JSON -> YOLO TXT 数据集转换流程")
    print("=" * 70)
